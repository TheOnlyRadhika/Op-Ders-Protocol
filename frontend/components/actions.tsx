...
export default function PoolStats() {
    ...
    const state = poolStats as any[]
    const poolBalance = state?.[0] ? formatBalance(state[0]) : '0'
    const totalEarned = state?.[1] ? formatBalance(state[1]) : '0'
    const lenderCount = state?.[2] ? Number(state[2]) : 0
    const platformFees = state?.[3] ? formatBalance(state[3]) : '0'
